...
class Test extends TestCase
{
    ...
    {
        // given
        $pattern = pattern('http://(?<name>[a-z]+)\.(com|org)');
        $subject = 'Links: http://google.com, http://other.org and http://website.org.';

        // when
        $result = $pattern->replace($subject)->all()->focus('name')->with('xxx');
    ...
    public function shouldGetFromReplaceMatch_all()
    {
        ...
        // when
        $pattern->replace($subject)->all()->callback(function (Detail $detail) {
            // then
    ...
    {
        // given
        $pattern = pattern('http://(?<name>[a-zę]+)\.(?<domain>com|org)');
        $subject = 'Links: http://googlę.com and http://other.org. and again http://danon.com';

        $offsets = [];

        $callback = function (Detail $detail) use (&$offsets) {
            $offsets[] = $detail->offset();
            return 'ę';
        };

        // when
        $pattern->replace($subject)->all()->callback($callback);
