 ...
 * groups (and to handling any group, actually, although in
 ...
 * in an array. So even if multiple groups (with duplicate
 * names) matched separate strings, only the last will be
 * present in the result. Thus, it's impossible to distinguish
 * which exact (indexed) group actually matched the named
 * group. It's possible to sometimes "guess" that certain
 * groups definitely didn't match it (if their values and/or
 * offsets are different), but if they're equal (and it's
 * possible that they're equal), then the distinguishment
 * is impossible.
 *
 * So there are two strategies to read a group value.
 * They have their advantages and flaws.
 *
 * - Read a group as it was at compiled pattern
 * - Read a group directly from the runtime result
 *
 * If you read a group at compile time, it becomes
 ...
 * first indexed group is named, and the other two aren't.
 *
 * Runtime strategy:
 ...
 * distinguish them, because PHP sucks big time.
 *
